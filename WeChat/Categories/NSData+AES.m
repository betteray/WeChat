...
static int aes_ecb_encrypt(const unsigned char *pKey, unsigned int uiKeyLen, const unsigned char *pInput, unsigned int uiInputLen, int bFinal, unsigned char **ppOutput, unsigned int *pOutputLen)
{
    ...
    if (pKey == NULL || uiKeyLen == 0 || pInput == NULL || uiInputLen == 0 || pOutputLen == NULL || (bFinal == 0 && (uiInputLen % AES_KEY_LEN) != 0) || ppOutput == NULL)
        return -1;
...
static int aes_ecb_decrypt(const unsigned char *pKey, unsigned int uiKeyLen, const unsigned char *pInput, unsigned int uiInputLen, int bFinal, unsigned char **ppOutput, unsigned int *pOutputLen)
{
    unsigned char keyBuf[AES_KEY_LEN] = {0};
